...
df = pd.read_csv('cart_pole_v4_v9.txt', sep = ';', names = ["a", "b", "c", "d"])
df2 = pd.read_csv('cart_pole_v4_high_crossover.txt', sep = ';', names = ["a", "b", "c", "d"])
...
generation2 = []
best_individual_score2 = []
second_best_individual2 = []
third_best_individual2 = []
average_generation_score2 = []
for index, row in df2.iterrows():
	values = row["b"].split('[')[1].split(']')[0]
	values = values.split(',')
	values = [float(element.strip()) for element in values]
	best_individual_score2.append(values[0])
	second_best_individual2.append(values[1])
	third_best_individual2.append(values[2])
	average_generation_score2.append(float(row["c"]))

plot_df = pd.DataFrame({
	"Best Individual in Generation": best_individual_score[:9],
	"Average Generation Score": average_generation_score[:9],
	"Best Individual in Generation - High Crossover": best_individual_score2[:9],
	"Average Generation Score - High Crossover": average_generation_score2[:9],
	})
